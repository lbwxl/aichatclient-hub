...
import { platformOptions, useWorkspaceStore } from '@aichat/core'
import { DoudianWebView } from '../features/douyin/DoudianWebView'
...
const PLATFORM_URL = 'https://im.jinritemai.com/pc_seller_v2/main/workspace'
...
function HomePage() {
  ...
  return <div className="relative flex h-full overflow-hidden bg-gradient-to-br from-slate-50/90 via-blue-50/80 to-slate-50/90">
    ...
    <main className="min-w-0 flex-1">{loadError && <div role="alert" className="absolute left-1/2 top-4 z-20 -translate-x-1/2 rounded-lg border border-rose-200 bg-rose-50 px-4 py-2 text-xs text-rose-600 shadow-sm">{loadError}</div>}{activeShop ? <DoudianWebView key={activeShop.id} shopId={activeShop.id} src={(activeShop.metadata.backend as { url?: string } | undefined)?.url || PLATFORM_URL} /> : <WelcomePanel onLogin={reloadShops} compact />}</main>
  </div>
}

function LoginSidebar({ onLogin }: { readonly onLogin: () => void }) {
  return <aside className="flex h-full w-80 shrink-0 flex-col border-r border-slate-200/70 bg-white/90 p-4 shadow-sm"><div className="border-b border-slate-100 pb-4"><p className="text-xs font-semibold tracking-widest text-blue-600">店铺管理</p><h2 className="mt-2 text-lg font-semibold text-slate-700">连接你的抖店</h2><p className="mt-1 text-xs leading-5 text-slate-400">登录后统一管理店铺消息、商品和订单。</p></div><button type="button" onClick={onLogin} className="mt-5 flex items-center justify-center gap-2 rounded-lg bg-gradient-to-r from-blue-500 to-blue-600 py-2.5 text-sm font-medium text-white shadow-sm transition hover:from-blue-600 hover:to-blue-700"><LogIn className="h-4 w-4" />一键登录</button><div className="mt-auto flex items-center gap-2 border-t border-slate-100 pt-4 text-xs text-slate-400"><Store className="h-4 w-4" />仅支持抖店平台</div></aside>
}

function WelcomePanel({ onLogin, compact = false }: { readonly onLogin: () => void; readonly compact?: boolean }) {
  return <div className="flex min-w-0 flex-1 items-center justify-center p-8 text-center"><div className={`animate-float-in ${compact ? 'max-w-sm' : 'max-w-lg'}`}><img src="/assets/images/jianXiaoZhiAi.png" alt="小二智客" className="mx-auto h-20 w-20 rounded-2xl object-contain" /><h2 className="mt-5 text-xl font-semibold text-slate-700">欢迎使用小二智客</h2><p className="mx-auto mt-3 max-w-md text-sm leading-7 text-slate-400">登录后即可连接抖店客服工作台，让店铺消息、商品与订单在一个窗口内流转。</p>{!compact && <button type="button" onClick={onLogin} className="mt-6 rounded-lg bg-blue-600 px-6 py-2.5 text-sm font-medium text-white shadow-sm hover:bg-blue-700">立即登录</button>}</div></div>
}
